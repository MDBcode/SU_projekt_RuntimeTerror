...
CYCLEGAN_MODEL_PATH = "models/saved_models/..."  # dodati model
CGAN_MODEL_PATH = "models/saved_models/..."  # dodati model

model_pix2pix = load_model(PIX2PIX_MODEL_PATH)
# model_cycleGAN = load_model(CYCLEGAN_MODEL_PATH)
...
def predict_cycleGAN(filename):
    img = load_image(filename)
    gen_image = model_cycleGAN.predict(img)
    gen_image = (gen_image + 1) / 2.0  # promijeniti
    return gen_image[0]  # promijeniti
