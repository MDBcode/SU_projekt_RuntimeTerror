...
CGAN_MODEL_PATH = "models/saved_models/our_cgan.h5"
...
def predict_cgan(filename):
    img = load_image(filename)
    predicted_img = model_cgan.predict(img)
    predicted_img = predicted_img.reshape((256,256,3))
    return (predicted_img + 1) / 2.0

def predict_pix2pix(filename):
    img = load_image(filename)
    predicted_img = model_pix2pix.predict(img)
    predicted_img = predicted_img.reshape((256, 256, 3))
    return (predicted_img + 1) / 2.0
...
def predict_cycleGAN_sat_to_map(filename):
    ...
    predicted_map = predicted_map.reshape((256, 256, 3))
    return (predicted_map + 1) / 2.0

def predict_cycleGAN_map_to_sat(filename):
    img = load_image(filename)
    predicted_satellite = model_cycleGAN_map_to_sat.predict(img)
    predicted_satellite = predicted_satellite.reshape((256, 256, 3))
    return (predicted_satellite + 1) / 2.0
